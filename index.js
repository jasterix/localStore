...
allItems.addEventListener("click", event => {
  if (event.target.className == "delete is-large") {
...
const makeLi = text => {
  ...
  newItem.innerHTML += `    <button class="delete is-large">X</button>`;
  ul.append(newItem);
  newItem.className = "title is-4 is-upper-alpha is-success";
};
